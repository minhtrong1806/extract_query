...
def _as_set(rows: List[dict[str, str]]) -> set[Tuple[str, str, str, str]]:
    return {(row.get("SCHEMA", ""), row.get("TABLE", ""), row.get("COLUMN", ""), row.get("REASON", "")) for row in rows}
...
def test_join_with_alias_and_schema():
    ...
    assert ("sch1", "table1", "col1", "") in row_set
    assert ("", "table2", "col2", "") in row_set
...
def test_cte_resolves_to_base_table():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("sch2", "t_users", "id", "") in row_set
...
def test_union_subquery_returns_multiple_sources():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("", "t_a", "col1", "") in row_set
    assert ("", "t_b", "col1", "") in row_set
...
def test_star_alias_fallback_for_unqualified_column():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("sch", "star_table", "col1", "") in row_set
...
def test_quoted_identifiers_schema_table():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("MYSCHEMA", "MYTABLE", "col", "") in row_set
...
def test_first_table_fallback_for_unqualified_column():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("", "t1", "col1", "FIRST_TABLE_FALLBACK") in row_set
...
def test_subquery_any_table_fallback_for_ambiguous_subquery():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("", "t1", "col1", "SUBQUERY_ANY_TABLE_FALLBACK") in row_set
...
def test_subquery_unresolved_reason():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("", "", "col1", "SUBQUERY_SOURCE_ONLY") in row_set
...
def test_unresolved_table_reason_for_unknown_alias():
    ...
    row_set = _as_set(rows)
    assert ("", "", "col1", "UNRESOLVED_TABLE") in row_set
...
def test_subquery_join_resolves_nested_column_alias():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("", "base_table", "verify_decision", "") in row_set
...
def test_cte_resolves_column_via_joined_subquery():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("", "base_table", "code", "") in row_set
...
def test_cte_union_all_resolves_base_tables_for_column():
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("KMDW", "FT_SV_CC_TRANS", "CUST_NUMBER", "") in row_set
    assert ("KMDW", "FT_SV_DC_TRANS", "CUST_NUMBER", "") in row_set
...
def test_derived_column_from_cte_chain_returns_derived_table():
    """Cột sinh ra từ CTE (không gắn bảng gốc) trả về __DERIVED__."""
    sql = """
        WITH c1 AS (
    ...
    rows = _extract_rows(sql)
    row_set = _as_set(rows)
    assert ("", "__DERIVED__", "STATUS", "DERIVED_COLUMN") in row_set
